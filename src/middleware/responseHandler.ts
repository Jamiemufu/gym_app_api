...
export const responseHandler = (resource: any, req: Request, res: Response) => {
  switch (req.method) {
    ...
    case "DELETE":
      return res.status(204).json({ status: 204, message: "Resource deleted successfully" });
    case "POST":
      return res.status(201).json({ status: 201, data: resource });
    case "PUT":
      return res.status(204).json({ status: 204, message: "Resource updated successfully", data: resource });
    case "PATCH":
      return res.status(204).json({ status: 204, message: "Resource updated successfully", data: resource });
    default:
      return res.status(200).json({ status: 200, data: resource });
